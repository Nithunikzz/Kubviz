...
func RunTrivyImageScan(config *rest.Config, js nats.JetStreamContext, wg *sync.WaitGroup, errCh chan error) {
	defer wg.Done()
...
func publishImageScanReport(report types.Report, js nats.JetStreamContext, errCh chan error) {
	metrics := model.Trivy{
		ID:          uuid.New().String(),
